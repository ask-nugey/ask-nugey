...
# proto完全ガイド：開発環境を統一する次世代ツールチェーンマネージャー

皆さんの開発環境には、いくつのバージョンマネージャーが入っていますか？
...
- 関連記事：[モノレポツールのmoonで構築したNestJSのデモアプリ](/posts/nestjs-with-moon-learn-demo)
...
graph LR
    ...
    proto --> standalone[スタンドアロンの`proto`として独立]
...
# 特定のツールで利用可能なバージョン一覧 (例: Bun)
proto list-remote bun
```
...
`status`コマンドで、今いるディレクトリでどのツールとバージョンが有効かを確認できます。
