...
https://zenn.dev/mizchi/articles/introduce-ts-similarity#%E4%BD%BF%E3%81%A3%E3%81%A6%E3%81%BF%E3%82%8B

特にVibe CodingでAIにコードを書かせていると類似コードが生産されがちということなので、定期的にsimilarity-tsを使うのが良さそうです。
...
一番簡単な使い方は[github上でも紹介](https://github.com/mizchi/similarity?tab=readme-ov-file#3-refactor-with-ai)されていますが、以下のプロンプトをClaude Codeなどにそのまま投げればいいです↓
...
（日本語訳↓）
```bash
`similarity-ts`コマンドを実行して、セマンティックなコードの類似性を検出してください。このコマンドを実行し、重複しているコードパターンを分析し、リファクタリングの計画を作成してください。詳細なオプションについては`similarity-ts -h`を確認してください。
```
